...
(function($) {
  ...
  var modificators = {
    ...
    // Set compile-time options from inside the template
    // Example: {%set name mytpl %} {%set args a,b,c %} {%set foo.bar 1,2,3 %}
    // The value is split into the first whitespace-delimited key and the
    // remainder; comma-lists become arrays, single values stay strings, and
    // dotted keys create nested objects on the options object.
    /** @return {string} empty (set produces no output). */
    'set': function(str, namespace, options) {
      var match = /^\s*(\S+)\s+([\s\S]*?)\s*$/.exec(str);
      if (!match || !options)
        return '';

      var raw = match[2];
      var value = raw.indexOf(',') !== -1 ?
          raw.split(',').map(function(s) { return s.replace(rtrim, ''); }) :
          raw;

      var keys = match[1].split('.'), target = options;
  ...
  // Trim used by the "set" modificator (leading/trailing whitespace + nbsp)
  /** @const */
  var rtrim = /^[\s ]+|[\s ]+$/g;
  ...
  *  @param {function(string,object): object} parse parse function.
  *  @param {function(string): undefined} callback Callback.
  *  @return {undefined} nothing.
  */
  function readTemplateAsync(filename, parse, callback) {

    // Check if file exists in async mode
    // (path.exists was removed from Node; fs.access is the modern equivalent)
    fs.access(filename, function(err) {
      ...
      // Get file contents
      fs.readFile(filename, function(err, data) {
        // If can't - treat the input as an inline template
        if (err)
          return callback(filename);
  ...
  *  @param {string} filename Template name.
  *  @param {function(string,object): object} parse parse function.
  *  @return {string} content or filename.
  */
  function readTemplateSync(filename, parse) {
    // If there is no file
    // "filename" isn't really filename, but template
    // So return it
    // (path.existsSync was removed from Node; fs.existsSync is the modern home)
  ...
  *    callback: function(generatedTemplate){}, // will load template from file
  *              asynchronously
  ...
  */
  var nTPL = $.parse = $.ntpl = $.nTPL = function(options) {
    ...
    if (_template)
      return $main(_template, _args || [], _name, options.callback,
                   options.watch, options.watchCallback);
  }
  /**
  * Generate, cache, return template
  * $main("name") - get cached template with name
  * $main("%template%", {args}, [name]) - generate template and optionally give
  *       it a name
  * Args = Template arguments
  * @param {string} str Input template, or template name.
  * @param {array.<string>} args Predefined arguments.
  * @param {string} name Template name.
  * @param {function(function(object): string): undefined} callback Callback for
  *       asynchronous mode.
  * @param {boolean} watch Watch file changes or not?.
  * @param {function(function(object): string): undefined} watchCallback
  *     When file is changed - this callback will be fired.
  * @return {function(object.<string, *>): string} renderer function.
  */
  function $main(str , args, name, callback, watch, watchCallback) {
    ...
    */
    function parse(str, callback) {
      ...
      if (hasCode = parsed &&
          (parsed.replVars.length > 1 || parsed.code.length > 0)) {
        // parsed = {replacements: [], code: []}
        var compiled = parsed.code.join('');
        var replVars = parsed.replVars;

        // Replacements can't be changed
        $rep = Object.freeze(parsed.replacements);
        ...
        try {
          ...
          i = scopedEval('(function(' + replVars + '){' +
              ' return (function($args,' + _args.join(',') + '){' +
              '  $_=[];' +
      ...
      * @param {object.<string, *>} callArgs Arguments that will be used now.
      * @return {array.<*>} generated arguments.
      */
      function createArguments(callArgs) {

        /** There're some predefined arguments such as:
        * $args = callArgs,
        * $scope = namespace,
        */
        var result = [callArgs], i;
      ...
      /**
      *  And cache it wrapper, that will recreate
      *  scope and call original function
